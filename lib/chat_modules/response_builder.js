...
import { convertChileTimeToUtc, getDayIdentifier, CHILE_UTC_OFFSET_HOURS } from '@/lib/chat_modules/dateTimeUtils.js'; // Necesario para buscar alternativas
...
export function buildCalendarResponse(
    ...
    busySlots, 
    currentYearChile, // Asegúrate que este valor se esté pasando desde el orquestador
    requestClientId
) {
  const { 
    ...
    TOMORROW_DATE_IDENTIFIER_CHILE, // Este se calcula en date_time_parser y se pasa en queryDetails
  ...
  if (isDebuggingQueryForResponseFind) {
    console.log(`\n🔍 DEBUGGING "MAÑANA (o día específico) 3PM" RESPONSE PREP (response_builder - ClientId: ${requestClientId}):`);
    ...
    console.log(`   TOMORROW_DATE_IDENTIFIER_CHILE: ${TOMORROW_DATE_IDENTIFIER_CHILE}`);
    console.log(`   targetDateForDisplay: ${targetDateForDisplay ? getDayIdentifier(targetDateForDisplay, 'America/Santiago') : 'null'}`);
    ...
    console.log(`🔍 END DEBUGGING "MAÑANA (o día específico) 3PM" RESPONSE PREP\n`);
  }
  ...
  if (targetHourChile !== null) { 
    ...
    const requestedSlotExactMatch = availableSlotsOutput.find(slotString => {
        ...
        if (isTargetingForFindCBLog) {
            ...
            if (indexOfHour > -1) {
                const excerpt = slotString.substring(indexOfHour, indexOfHour + 12); 
                let charCodes = "";
        ...
        if (parts.length < 2) { 
            if (isTargetingForFindCBLog) console.log(`🔍 DEBUG FIND CB: parts.length < 2 para "${slotString}"`);
            return false; 
        }
    
        const timeChunk = parts[parts.length - 1].trim(); 
        // Regex CORREGIDO para manejar "a. m." o "p. m." (con espacio y puntos)
