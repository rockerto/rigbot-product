...
export function parseDateTimeQuery(lowerMessage, effectiveConfig, serverNowUtc, refDateForTargetCalc, requestClientId) {
    ...
    if (timeMatch) {
      ...
      if (!specificDateParsed || (specificDateParsed && (timeMatch[2] || timeMatch[4]))) {
          ...
          console.log(`DEBUG (date_time_parser): timeMatch capturó un número (${timeMatch[1]}) pero se ignoró. ClientId: ${requestClientId}`);
      }
    }
    ...
    if (targetHourChile !== null) { 
      ...
      if (!WORKING_HOURS_CHILE_NUMERIC.includes(requestedTimeNumeric) || requestedTimeNumeric < 10 || requestedTimeNumeric > 19.5) {
          ...
          let reply = `${replyPreamble} está fuera de nuestro horario de atención (${effectiveConfig.horario}). ¿Te gustaría buscar dentro de ese rango?${getWhatsappContactMessage(effectiveConfig.whatsappNumber, WHATSAPP_FALLBACK_PLACEHOLDER)}`;
          console.log('✅ Respuesta generada (fuera de horario) por date_time_parser:', reply);
          return { earlyResponse: { response: reply, status: 200 } };
      }
    }
