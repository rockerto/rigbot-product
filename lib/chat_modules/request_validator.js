...
export async function validateRequest(req, res) {
  // ... (Lógica de CORS y Method Check - SIN CAMBIOS)
  ...
  const { 
    ...
    clave: incomingClave,
    conversationHistory: incomingConversationHistory, // <--- NUEVO: Extraer del body
    sessionState: incomingSessionState // <--- NUEVO: Extraer del body
  } = req.body || {};
  // === FIN DE CAMBIO ===
  ...
  // NO loguear el mensaje del usuario aquí, se hace en el orquestador chat.js DESPUÉS de validar todo.
  // Lo movimos para que el logRigbotMessage de user se haga una sola vez.
  ...
  return {
    ...
    requestData: { 
        ...
        ipAddress,
        conversationHistory: incomingConversationHistory, // <--- NUEVO: Pasar al orquestador
        sessionState: incomingSessionState             // <--- NUEVO: Pasar al orquestador
    }
    // === FIN DE CAMBIO ===
  };
}